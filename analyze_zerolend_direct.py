"""
Analyze ZEROLEND Spot Market - Direct API Version
    ...
    def get_orderbook(self, market: str, limit: int = 20) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            params = {
                "market": market,
                "limit": str(limit),
                "interval": "0"  # Some APIs require interval parameter
            }
            response = self.session.get(url, params=params, timeout=10)
            response.raise_for_status()
            data = response.json()
...
def analyze_zerolend():
    ...
    print(f"\n{'='*70}")
    print(f"ZEROLEND Spot Market Analysis (Direct API)")
    ...
    if ticker:
        ...
        print(f"   24h Volume: {volume_24h:,.2f} ZEROLEND")
    ...
    if ticker and volume_24h < 10000:
        health_score -= 20
        issues.append(f"Low 24h volume ({volume_24h:,.0f} ZEROLEND)")
