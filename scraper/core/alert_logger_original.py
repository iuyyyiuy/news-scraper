...
class AlertLogger:
    ...
    def _store_log_entry(self, entry: AlertLogEntry):
        ...
        try:
            data = entry.to_dict()
            response = self.db_manager.supabase.table('alert_logs').insert(data).execute()
            # Success - no need for fallback
        except Exception as e:
            # Check if it's a 404 (table doesn't exist) or other error
            error_str = str(e).lower()
            if '404' in error_str or 'not found' in error_str or 'relation' in error_str:
                print(f"⚠️ Alert tables not found - using file fallback. Run 'python fix_alert_tables_404.py' to create tables.")
            else:
    ...
    def _store_session_stats(self):
        ...
        try:
            ...
            response = self.db_manager.supabase.table('scraping_sessions').insert(data).execute()
            # Success - no need for fallback
        except Exception as e:
            # Check if it's a 404 (table doesn't exist) or other error
            error_str = str(e).lower()
            if '404' in error_str or 'not found' in error_str or 'relation' in error_str:
                print(f"⚠️ Session tables not found - using file fallback. Run 'python fix_alert_tables_404.py' to create tables.")
            else:
                print(f"⚠️ Database error storing session stats: {e}")
            ...
            try:
                ...
                print(f"📁 Session stats saved to {session_file}")
    ...
    def _fallback_file_log(self, entry: AlertLogEntry):
        ...
        try:
            ...
            if not hasattr(self, '_fallback_logged'):
                print(f"📁 Alerts saved to {log_file} (database unavailable)")
