...
class SchedulerService:
    ...
    def schedule_daily_scrape(self):
        """
        Schedule daily scraping at 8:00 AM UTC+8
        """
        try:
            # Schedule for 8:00 AM every day (Asia/Shanghai timezone)
            self.scheduler.add_job(
                func=self._run_daily_scrape,
                trigger=CronTrigger(hour=8, minute=0, timezone=pytz.timezone('Asia/Shanghai')),
                id='daily_scrape',
                name='Daily News Scraping',
                replace_existing=True
            )
            print("✅ Scheduled daily scrape at 8:00 AM UTC+8")
