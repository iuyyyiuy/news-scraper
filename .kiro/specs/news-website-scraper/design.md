...
The system now supports three major crypto news sources:

1. **BlockBeats** (theblockbeats.info) - Uses sequential article IDs
2. **Jinse** (jinse.cn) - Uses sequential article IDs  
3. **PANews** (panewslab.com) - Uses alphanumeric article IDs

Each source has a dedicated scraper that follows the same interface pattern.

### Multi-Source Coordinator

The `MultiSourceScraper` class coordinates scraping from multiple sources:

**Responsibilities:**
- Manage multiple scraper instances
- Support parallel or sequential scraping
- Aggregate results from all sources
- Apply deduplication across sources
- Track per-source statistics

**Key Methods:**
- `scrape(parallel: bool) -> ScrapingResult` - Execute multi-source scraping
- `get_source_results() -> Dict[str, ScrapingResult]` - Get per-source statistics
- `get_source_articles() -> Dict[str, List[Article]]` - Get per-source articles

### Deduplication Engine

The `DeduplicationEngine` detects and filters duplicate articles using content similarity:

**Similarity Signals:**
- **Title Similarity** (primary): 85% threshold
- **Body Similarity** (secondary): 80% threshold  
- **Combined Score**: 75% threshold (60% title + 40% body)

**Algorithm:**
1. Normalize text (lowercase, remove punctuation, trim)
2. Calculate similarity using SequenceMatcher
3. Compare each article against unique articles
4. Keep earliest published version when duplicates found

**Key Methods:**
- `deduplicate(articles: List[Article]) -> List[Article]` - Remove duplicates
- `is_duplicate(article1, article2) -> Tuple[bool, float, dict]` - Check if duplicate
- `calculate_similarity(text1, text2) -> float` - Calculate text similarity
- `get_statistics() -> dict` - Get deduplication statistics
...
```python
class SourceScraper:
    def __init__(config, data_store, start_date, end_date, keywords_filter, 
                 progress_callback, log_callback)
    def find_latest_article_id() -> Optional[int]
    def scrape() -> ScrapingResult
```

**Common Features:**
- Article ID iteration (backwards from latest)
...
- Each scraper defines CSS selectors for its target website
- Selectors passed to HTMLParser for content extraction
- Fallback logic when selectors don't match

## Future Enhancements

- ✅ Multi-site scraping with site-specific adapters (COMPLETED)
- ✅ Article deduplication using content similarity (COMPLETED)
- Support for JavaScript-rendered content (Selenium/Playwright)
- Advanced similarity algorithms (TF-IDF, BERT embeddings)
